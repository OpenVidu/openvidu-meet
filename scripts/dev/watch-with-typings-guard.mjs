...
// Configuration
const FLAG_PATH = resolve(__dirname, '../../typings/dist/typings-ready.flag');
const TYPINGS_DIST = resolve(__dirname, '../../typings/dist');
...
function checkTypingsReady() {
	...
	isTypingsReady = existsSync(FLAG_PATH);
 ...
 */
function watchFlag() {
	console.log(`Watching typings flag: ${FLAG_PATH}`);

	// Initial check
	checkTypingsReady();

	// Watch the parent directory of the flag
	const flagDir = dirname(FLAG_PATH);
...
function watchTypingsDist() {
	console.log(`Watching typings dist: ${TYPINGS_DIST}`);

	const watcher = watch(TYPINGS_DIST, { recursive: true }, (eventType, filename) => {
