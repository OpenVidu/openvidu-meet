...
/**
 * Covers `closeOrLeave`'s EMBEDDED_CLOSE_REQUESTED host-event emission gating in embedded modes (the SPA
...
describe('NavigationService - hosted-mode event gates', () => {
	...
	describe('goBackFromMeeting() → closeOrLeave()', () => {
		it('iframe mode with no leave-redirect: emits EMBEDDED_CLOSE_REQUESTED and does not navigate', async () => {
			iframeMode = true;

			await service.goBackFromMeeting('/rooms');

			expect(eventBus.emit).toHaveBeenCalledOnceWith({ event: EmbeddedEventName.EMBEDDED_CLOSE_REQUESTED });
			expect(router.navigate).not.toHaveBeenCalled();
		});

		it('iframe mode with leave-redirect: emits EMBEDDED_CLOSE_REQUESTED and redirects', async () => {
			iframeMode = true;
