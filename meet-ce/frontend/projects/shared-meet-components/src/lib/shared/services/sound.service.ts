...
export class SoundService {
	...
	playParticipantJoinedSound(): void {
		const audio = new Audio(this.runtimeConfig.resolvePath('assets/sounds/participant-joined.mp3'));
		audio.volume = 0.4;
		audio.play();
	}
	...
	playParticipantRoleUpgradedSound(): void {
		const audio = new Audio(this.runtimeConfig.resolvePath('assets/sounds/role-upgraded.wav'));
		audio.volume = 0.4;
		audio.play();
	}
	...
	playParticipantRoleDowngradedSound(): void {
		const audio = new Audio(this.runtimeConfig.resolvePath('assets/sounds/role-downgraded.wav'));
		audio.volume = 0.4;
		audio.play();
	}
}
