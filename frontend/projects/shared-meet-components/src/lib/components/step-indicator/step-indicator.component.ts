...
export class StepIndicatorComponent implements OnChanges {
  ...
  @Output() stepClick = new EventEmitter<{ step: WizardStep; index: number }>();  visibleSteps: WizardStep[] = [];
  ...
  getStepControl(step: WizardStep): FormControl {
    return this.stepControls[step.id] || new FormControl();
  }
